...
if page == "📂 연구 종합 보고서":
    ...
    <b>개발 도구</b>: <b>Streamlit</b>(웹 프레임워크) & <b>GitHub</b>(버전 관리)<br>
    <b>참고 데이터</b>: 한글(HWP) 파일 내부 XML 패키지 구조 분석[cite: 1]<br>
    <b>연구 목적</b>: 비밀번호 보안성을 수학적 정보량과 공격 저항성 기반으로 정량 평가.
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        steps = [
            "1단계 : 데이터 패키지 구조 분석(XML/HWP 등)[cite: 1]", 
    ...
    with col2:
        st.markdown('<div class="card"><h3>📌 핵심 기술 스택</h3><ul><li><b>데이터 구조:</b> HWP의 XML 파일 시스템 및 메타데이터 구성 분석[cite: 1]</li><li><b>Streamlit:</b> 대화형 데이터 분석 웹 프레임워크</li><li><b>GitHub:</b> 코드 버전 관리 및 배포</li></ul></div>', unsafe_allow_html=True)

    st.subheader("🎯 3. 결론 (종합 결과)")
    k1, k2, k3 = st.columns(3)
    k1.markdown('<div class="card" style="border-top:5px solid #38bdf8"><h4>암호학적 효율성</h4>지수적 길이 증가가 공격자의 계산 복잡도를 압도함.</div>', unsafe_allow_html=True)
    k2.markdown('<div class="card" style="border-top:5px solid #fbbf24"><h4>인지적 효율성</h4>패스프레이즈 모델로 기억 부하와 보안성 동시 확보.</div>', unsafe_allow_html=True)
    k3.markdown('<div class="card" style="border-top:5px solid #22c55e"><h4>보안적 경제성</h4>12자 이상의 패스프레이즈를 통한 경제적 방어선 구축.</div>', unsafe_allow_html=True)
